...
export default function Manager(){
  ...
  return <Shell authenticated logout={logout}>
    <main className="manager">
      ...
      <div className="legend"><i className="recent"/>New in last 48 hours <i className="employee"/>Employee submission <i className="parent-note"/>Parent note <i className="note"/>Manager note</div>
      <div className="week">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(x=><b key={x}>{x}</b>)}</div>
      <div className="calendar">{cells.map((d,i)=>{const date=d?`${key(month)}-${pad(d)}`:'';return <div className="day" key={i}>{d&&<><div className="daytop"><b>{d}</b><button className="plus" title="Add calendar note" onClick={()=>setModal({kind:'note',event_date:date,title:'',note_type:'Leaving Early',note_category:'Manager Note',details:''})}>+ Note</button></div>{entries.filter(x=>x.event_date===date&&x.submitter_type==='Employee').map(x=><button className={`event ${x.submitter_type.toLowerCase()}${isNew(x.submitted_at)?' recent':''}`} key={'e'+x.id} onClick={()=>setModal({...x,kind:'entry'})}><small>{x.submitter_type}{isNew(x.submitted_at)?' · New':''}</small>{x.name} — {x.request_type}</button>)}{notes.filter(x=>x.event_date===date).map(x=>{const category=x.note_category||'Manager Note';return <button className={`event note ${category==='Parent Note'?'parent-note':'manager-note'}${isNew(x.created_at)?' recent':''}`} key={'n'+x.id} onClick={()=>setModal({...x,note_category:category,kind:'note'})}><small>{category}{isNew(x.created_at)?' · New':''}</small>{x.title} — {x.note_type}</button>})}</>}</div>})}</div>
    </main>
