...
export default function Manager(){
  ...
  return <Shell authenticated logout={logout}>
    <main className="manager">
      ...
      <div className="legend"><i className="recent"/>New in last 48 hours <i className="employee"/>Employee submission <i className="parent"/>Parent submission <i className="note"/>Manager note</div>
      <div className="week">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map(x=><b key={x}>{x}</b>)}</div>
      <div className="calendar">{cells.map((d,i)=>{const date=d?`${key(month)}-${pad(d)}`:'';return <div className="day" key={i}>{d&&<><div className="daytop"><b>{d}</b><button className="plus" title="Add manager note" onClick={()=>setModal({kind:'note',event_date:date,title:'',note_type:'Leaving Early',details:''})}>+ Note</button></div>{entries.filter(x=>x.event_date===date).map(x=><button className={`event ${x.submitter_type.toLowerCase()}${isNew(x.submitted_at)?' recent':''}`} key={'e'+x.id} onClick={()=>setModal({...x,kind:'entry'})}><small>{x.submitter_type}{isNew(x.submitted_at)?' · New':''}</small>{x.name} — {x.request_type}</button>)}{notes.filter(x=>x.event_date===date).map(x=><button className={`event note${isNew(x.created_at)?' recent':''}`} key={'n'+x.id} onClick={()=>setModal({...x,kind:'note'})}><small>Manager note{isNew(x.created_at)?' · New':''}</small>{x.title} — {x.note_type}</button>)}</>}</div>})}</div>
    </main>
    {modal&&<div className="overlay"><form className="card modal" onSubmit={save}><h2>{modal.id?'Edit':'Add'} {modal.kind==='note'?'calendar note':'entry'}</h2>{modal.kind==='entry'?<><Field t="Submitting as"><select value={modal.submitter_type} onChange={e=>setModal({...modal,submitter_type:e.target.value})}><option>Employee</option><option>Parent</option></select></Field><Field t="Name"><input value={modal.name} onChange={e=>setModal({...modal,name:e.target.value})}/></Field><Field t="Request type"><select value={modal.request_type} onChange={e=>setModal({...modal,request_type:e.target.value})}>{['Called Out','PTO','Sick Leave','Other'].map(x=><option key={x}>{x}</option>)}</select></Field><Field t="Details"><textarea value={modal.reason||''} onChange={e=>setModal({...modal,reason:e.target.value})}/></Field></>:<><Field t="Person or title"><input required value={modal.title} onChange={e=>setModal({...modal,title:e.target.value})}/></Field><Field t="Occasion"><select value={modal.note_type} onChange={e=>setModal({...modal,note_type:e.target.value})}>{['Leaving Early','Coming Late','Appointment','Schedule Change','Special Occasion','Other'].map(x=><option key={x}>{x}</option>)}</select></Field><Field t="Notes"><textarea value={modal.details||''} onChange={e=>setModal({...modal,details:e.target.value})}/></Field></>}<Field t="Date"><input required type="date" value={modal.event_date} onChange={e=>setModal({...modal,event_date:e.target.value})}/></Field><div className="actions"><button>Save</button>{modal.id&&<button type="button" className="danger" onClick={()=>remove(modal.kind==='note'?'notes':'manager',modal.id)}>Delete</button>}<button type="button" className="secondary" onClick={()=>setModal(null)}>Cancel</button></div></form></div>}
  </Shell>;
}
